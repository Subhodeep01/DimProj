...
# -------------------------------------------------------------------------
# Auto‑encoder with DEC/IDEC‑style clustering
# -------------------------------------------------------------------------
class Autoencoder(nn.Module):
    def __init__(self, *, cluster_num:int, t_alpha:float, alpha:float, gamma:float,
                 lr_rate:float, input_dim:int = 784, hidden_dim:int = 128,
        ...
        # simple fully‑connected AE (He init) --------------------------------
    ...
    # ------------------------------------------------------------------
    def pretrain(self, train_loader: DataLoader, pretrain_epoch:int):
        print("[pre‑train] starting …")
        self.to(DEVICE)
        ...
        # keep on CPU for sklearn
        self.latent_repre = self.latent_repre.cpu().numpy()

    # ------------------------------------------------------------------
    # stage‑2: joint fine‑tuning with clustering
    # ------------------------------------------------------------------
    def finetrain(self, train_loader: DataLoader, train_epoch:int, update_epoch:int, *, error:float=1e-3):
        print("[fine‑train] starting …")
        self.to(DEVICE)
        opt = optim.Adam(self.parameters(), lr=self.lr_rate)

        # --- (1) initial K‑means on pretrained encodings -----------------
        ...
        for epoch in range(train_epoch):
            # 2.A – full‑dataset assignment update every `update_epoch` ----
